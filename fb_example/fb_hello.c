...
int main(int argc, char *argv[])
{
	...
	printf("Screen resolution: (%dx%d) yoffset = %d\n", var_info.xres, var_info.yres, var_info.yoffset);
	...
	printf("var_info.activate = %d\n", var_info.activate);
	
 
	// FBIOPUT_VSCREENINFO required to make fb work (on android at least)
	// ioctl do nothing if var_info wasn't changed, so we need to do smt with it twice. yoffset is pretty safe to manipulate

        var_info.yoffset = 1;

	if (ioctl(FrameBufferFD, FBIOPUT_VSCREENINFO, &var_info) < 0) {
		printf("Unable to write back variable screen info: %s\n",strerror(errno));
		close(FrameBufferFD);
		return 1;
        }
	...
	if (fix_info.visual == FB_VISUAL_TRUECOLOR) {
		...
		*/
		pixel = ((red >> (8 - var_info.red.length))     << var_info.red.offset) |
	...
	switch (var_info.bits_per_pixel) {
		case 8:
			*((u_int8_t *)framebuffer + fix_info.line_length * y + x) = (u_int8_t)pixel;
			break;
		case 16:
			*((u_int16_t *)framebuffer + fix_info.line_length * y + x * 2) = (u_int16_t)pixel;
			break;
		case 32:
			*((u_int32_t *)(framebuffer + fix_info.line_length * y + x * 4)) = (u_int32_t)pixel;
			break;
		default:
