...
static inline void in8 (ioport_t port, uint8_t *data)
{ asm volatile ("inb %%dx, %%al" : "=a" (*data) : "d" (port)); }
static inline void in16 (ioport_t port, uint16_t *data)
...
static inline void ins8 (ioport_t port, uint8_t *buf, uint32_t count)
{ asm volatile ("cld; rep insb" : "=c" (count), "=D" (buf)
                : "c" (count), "d" (port), "D" (buf)); }
static inline void ins16 (ioport_t port, uint16_t *buf, uint32_t count)
{ asm volatile ("cld; rep insw" : "=c" (count), "=D" (buf)
                : "c" (count), "d" (port), "D" (buf)); }
static inline void ins32 (ioport_t port, uint32_t *buf, uint32_t count)
{ asm volatile ("cld; rep insl" : "=c" (count), "=D" (buf)
                : "c" (count), "d" (port), "D" (buf)); }
static inline void insn (ioport_t port, void *buf, int unit_size, uint32_t total_size)
{
        if (unit_size == 4)
...
static inline void out8 (ioport_t port, uint8_t data)
{ asm volatile ("outb %%al, %%dx" : : "a" (data), "d" (port)); }
static inline void out16 (ioport_t port, uint16_t data)
{ asm volatile ("outw %%ax, %%dx" : : "a" (data), "d" (port)); }
static inline void out32 (ioport_t port, uint32_t data)
{ asm volatile ("outl %%eax, %%dx" : : "a" (data), "d" (port)); }
static inline void outs8 (ioport_t port, uint8_t *buf, uint32_t count)
{ asm volatile ("cld; rep outsb" : "=c" (count), "=S" (buf)
                : "c" (count), "d" (port), "S" (buf)); }
static inline void outs16 (ioport_t port, uint16_t *buf, uint32_t count)
{ asm volatile ("cld; rep outsw" : "=c" (count), "=S" (buf)
                : "c" (count), "d" (port), "S" (buf)); }
static inline void outs32 (ioport_t port, uint32_t *buf, uint32_t count)
{ asm volatile ("cld; rep outsl" : "=c" (count), "=S" (buf)
                : "c" (count), "d" (port), "S" (buf)); }
static inline void outsn (ioport_t port, void *buf, int unit_size, uint32_t total_size)
{
        if (unit_size == 4)
