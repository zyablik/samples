...
// ------------------------------------------------------------------------------------------------
    uint8_t * batchBuffer = align(memMgr.gfxMemNext, 4096);
    size = 4 * KB;
    memMgr.gfxMemNext = batchBuffer + size;
    memset(batchBuffer, 0, size);

    printk("batchBuffer = %p size = %d\n", batchBuffer, size);
    uint32_t * ptr = (uint32_t *)batchBuffer;
    uint8_t * state = batchBuffer + size / 2;

    printf("batchBuffer = %p gfxbb: %p state = %p end = %p\n", batchBuffer, batchBuffer - memMgr.gfxMemBase, state, batchBuffer + sizeof(batchBuffer));
...
#define GEN7_PIPE_CONTROL_DEPTH_CACHE_FLUSH	(1 << 0)
    ...
    *ptr++ = (batchBuffer - memMgr.gfxMemBase) | BASE_ADDRESS_MODIFY;
    ...
    *ptr++ = ((uint8_t *)scv_state - batchBuffer);
...
//    printf("(uint8_t *)blend - batchBuffer = %d sizeof(blend) = %d state = %p\n", (uint8_t *)blend - batchBuffer, sizeof(gen7_blend_state), state);
    *ptr++ = ((uint8_t *)blend - batchBuffer);
    ...
    *ptr++ = ((uint8_t *)cc_state - batchBuffer) | 1;
    ...
    *ptr++ = ((uint8_t *)depth_stencil_state - batchBuffer);
    ...
    *ptr++ = (GEN7_3DSTATE_BINDING_TABLE_POINTERS_PS | (2 - 2));
    ...
    /// binding_table[0] = gen7_bind_buf(batch, dst, GEN7_SURFACEFORMAT_B8G8R8A8_UNORM, 1);
    {
        ...
        binding_table[0] = (uint8_t *)ss - batchBuffer;
    }

    *ptr++ = (uint8_t *) binding_table - batchBuffer;
    ...
    *ptr++ = ((uint8_t *)scissor_rect - batchBuffer);
...
//------------------------------------------------------------------------------------
    ...
    IoWriteMmReg32(gfx_pci.mmioBarVirt + RCS_RING_BUFFER_START, batchBuffer - memMgr.gfxMemBase);
...
int i = 0;
printk("batch buffer: batchBuffer = %p ptr = %p\n", batchBuffer, ptr);
for(uint32_t * tmp_ptr = batchBuffer; tmp_ptr < ptr; tmp_ptr++, i++) {
    printk("%4d: %p\n", i, *tmp_ptr);
}

    IoWriteMmReg32(gfx_pci.mmioBarVirt + RCS_RING_BUFFER_TAIL, (uint8_t *)ptr - batchBuffer);
