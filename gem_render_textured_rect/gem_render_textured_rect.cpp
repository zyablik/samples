...
int main(int argc, char ** argv) {
    ...
    drm_intel_bo * texture_bo = drm_intel_bo_alloc(bufmgr, "texture_bo", 4096, 4096);
    printf("drm_intel_bo_alloc(texture_bo, %d): texture_bo = %p\n", 4096, texture_bo);
    ...
    /// gem_write(drm_fd, texture_bo->handle, 0, linear, sizeof(linear));
    struct drm_i915_gem_pwrite gem_pwrite_texture = { 0 };
    gem_pwrite_texture.handle = texture_bo->handle;
    gem_pwrite_texture.size = sizeof(pixels);
    gem_pwrite_texture.data_ptr = (uintptr_t)&pixels;
    ret = ioctl(dri_fd, DRM_IOCTL_I915_GEM_PWRITE, &gem_pwrite_texture);
    if (ret == -1) {
        printf("error while ioctl(DRM_IOCTL_I915_GEM_PWRITE, texture_bo->handle): %d:%s\n", errno, strerror(errno));
        exit(EXIT_FAILURE);
    }

    printf("ioctl(DRM_IOCTL_I915_GEM_PWRITE, texture_bo->handle): filled\n");
...
//    printf("(uint8_t *)blend - batch_buffer = %d sizeof(blend) = %d state = %p\n", (uint8_t *)blend - batch_buffer, sizeof(gen7_blend_state), state);
    ...
    *ptr++ = (GEN7_3DSTATE_BINDING_TABLE_POINTERS_PS | (2 - 2));
    ...
    }
    {
        ...
        ss[1] = texture_bo->offset;

        ret = drm_intel_bo_emit_reloc(batch_bo, (uint8_t *)ss - batch_buffer + 4, texture_bo, 0, I915_GEM_DOMAIN_RENDER, I915_GEM_DOMAIN_RENDER);
        if (ret != 0) {
            printf("error while drm_intel_bo_emit_reloc(I915_GEM_DOMAIN_RENDER, I915_GEM_DOMAIN_RENDER): %d:%s\n", errno, strerror(errno));
            exit(EXIT_FAILURE);
        }
    ...
    *ptr++ = (0 << GEN7_VB0_BUFFER_INDEX_SHIFT | GEN7_VB0_VERTEXDATA | GEN7_VB0_ADDRESS_MODIFY_ENABLE | 16 << GEN7_VB0_BUFFER_PITCH_SHIFT);
    ...
    uint32_t offset_begin = (uint8_t *)v - batch_buffer;

    /// OUT_RELOC(batch->bo, I915_GEM_DOMAIN_VERTEX, 0, offset_begin);
    ret = drm_intel_bo_emit_reloc(batch_bo, (uint8_t *)ptr - (uint8_t *)batch_buffer, batch_bo, offset_begin, I915_GEM_DOMAIN_VERTEX, 0);
    if (ret != 0) {
        printf("error while drm_intel_bo_emit_reloc(I915_GEM_DOMAIN_VERTEX): %d:%s\n", errno, strerror(errno));
        exit(EXIT_FAILURE);
    }
    *ptr++ = batch_bo->offset64 + offset_begin;

    // uint32_t offset_end = offset_begin + 16 * sizeof(*v) - 1;
    // /// OUT_RELOC(batch->bo, I915_GEM_DOMAIN_VERTEX, 0, offset_end);
    // ret = drm_intel_bo_emit_reloc(batch_bo, (uint8_t *)ptr - (uint8_t *)batch_buffer, batch_bo, offset_end, I915_GEM_DOMAIN_VERTEX, 0);
    // if (ret != 0) {
    //     printf("error while drm_intel_bo_emit_reloc(I915_GEM_DOMAIN_VERTEX): %d:%s\n", errno, strerror(errno));
    //     exit(EXIT_FAILURE);
    // }
    *ptr++ = ~0; //batch_bo->offset64 + offset_end;
    *ptr++ = 0;
    ...
    *ptr++ = (1 << GEN7_VB0_BUFFER_INDEX_SHIFT | GEN7_VB0_VERTEXDATA | GEN7_VB0_ADDRESS_MODIFY_ENABLE | 8 << GEN7_VB0_BUFFER_PITCH_SHIFT);
    ...
    offset_begin = (uint8_t *)uv - batch_buffer;
    ...
    *ptr++ = batch_bo->offset64 + offset_begin;
    ...
    // offset_end = offset_begin + 8 * sizeof(*uv) - 1;
    // /// OUT_RELOC(batch->bo, I915_GEM_DOMAIN_VERTEX, 0, offset_end);
    // ret = drm_intel_bo_emit_reloc(batch_bo, (uint8_t *)ptr - (uint8_t *)batch_buffer, batch_bo, offset_end, I915_GEM_DOMAIN_VERTEX, 0);
    // if (ret != 0) {
    //     printf("error while drm_intel_bo_emit_reloc(I915_GEM_DOMAIN_VERTEX): %d:%s\n", errno, strerror(errno));
